...
export class FavoritesView extends Favorites {
    ...
    onadd() {
        const addButton = this.root.querySelector('.container-add-fav button')
        addButton.onclick = () => {
            const { value } = this.root.querySelector('.container-add-fav input')
    ...
    update() {
        this.removeAllTr()
            ...
            if(this.entries.length === 0) {
                let trEmpt = document.createElement('tr')
                trEmpt.classList.add('noFav')
                trEmpt.innerHTML = `
                <td colspan="4">
                    <div>
            ...
            `
            return this.tbody.append(trEmpt)
        ...
        this.entries.forEach( user => {
            ...
            row.querySelector('.name-complete').textContent = user.name
